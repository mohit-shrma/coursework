...
void ComputeNeighbors(params_t *params)
{
  ...
  for (i=0; i<mat->nrows; i++) {
    ...
    nhits = gk_csr_GetSimilarRows(mat, 
				  mat->rowptr[i+1]-mat->rowptr[i], //no. of elements in row
				  mat->rowind+mat->rowptr[i], //index to column location of non zero values in row
				  mat->rowval+mat->rowptr[i], //index to the nonzero values in row
