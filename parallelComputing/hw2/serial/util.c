...
int gk_dfkvkselect(size_t n, int topk, gk_fkv_t *cand)
{
  ...
  //TODO: whats happening here ?? is array sorted?
        ...
        gk_fkv_t *i_cand)
{
  ...
  //marker to mark the candidate no-zero rows only
  marker = (i_marker ? i_marker : gk_ismalloc(nrows, -1, "gk_csr_SimilarRows: marker"));
  //store the nonzero hits and partial sum
  cand   = (i_cand   ? i_cand   : gk_fkvmalloc(nrows, "gk_csr_SimilarRows: cand"));

  switch (simtype) {
    case GK_CSR_COS:
      //for elements in column of specified row
      for (ncand=0, ii=0; ii<nqterms; ii++) {
	//get nonzero col index in row
        i = qind[ii]; 
	//colptr is similar to rowptr it stores the range of elements in column
	//of elements in?? TODO: verify
        for (j=colptr[i]; j<colptr[i+1]; j++) {
	  //for all the elements in the column i
	  //get the actual row of these elements
          k = colind[j];
	  //marker to record non zero row hit and store partial sum of 
	  //hits as key in it
          if (marker[k] == -1) {
	    //if the marker corresponding to row is not set
	    //record the row corresponding to column where first
	    //non zero value found
	    //store the first non-zero row hit row, mark it as candidate 
            cand[ncand].val = k;
            cand[ncand].key = 0;
	    //unset the corresponding flag and increment the non-zero row candidate hit count
	    marker[k] = ncand++;
          }
	  //store the partial sum corresponding to the row hit
	  /*
	      a b c : d1
	      d e f : d2
	      part sum = ad then +be then +cf 
	   */
  ...
  for (j=0, i=0; i<ncand; i++) {
    //unset the corresponding row for next doc similarity executions
    marker[cand[i].val] = -1; 
